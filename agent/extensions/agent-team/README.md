...
- 角色键名会去除首尾空格后再匹配。
- 无法解析的 JSON、非对象根节点静默忽略；覆盖键在适用作用域中没有对应 Markdown 角色时会被忽略（`both` 作用域下，项目级覆盖可以匹配同名用户级 Markdown）。
- `user` 作用域只应用用户级覆盖，`project` 作用域只应用项目级覆盖。`both` 作用域下，同名项目 Markdown 角色整体优先并使用项目级覆盖；若项目中没有同名 Markdown，则项目级覆盖会按字段应用到同名用户级角色（角色来源、描述和正文仍来自用户级 Markdown）。
